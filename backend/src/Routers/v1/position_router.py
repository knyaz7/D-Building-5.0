...
async def get_position(position_id: int, token: str = Depends(oauth2_scheme), session: AsyncSession = Depends(get_session)):
    await AuthController.verify_token(token, session)
    return await PositionController.get_one(position_id, session)


@router.post("/", response_model=PositionOutput)
async def create_position(position: PositionInput, token: str = Depends(oauth2_scheme), session: AsyncSession = Depends(get_session)):
...
async def update_position(position_id: int, position_data: PositionInput, token: str = Depends(oauth2_scheme), session: AsyncSession = Depends(get_session)):
    await AuthController.verify_token(token, session, 1)
    return await PositionController.update(position_id, position_data, session)


@router.delete("/{position_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_position(position_id: int, token: str = Depends(oauth2_scheme), session: AsyncSession = Depends(get_session)):
    await AuthController.verify_token(token, session, 1)
