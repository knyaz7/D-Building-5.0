...
router = APIRouter(prefix="/api/v1/master_task", tags=["Master tasks"])
...
@router.get("/{user_id}", response_model=MasterTaskOutput)
...
@router.put("/{user_id}", response_model=MasterTaskOutput)
async def update_user(task_id: int, task_data: MasterTaskInput, session: AsyncSession = Depends(get_session)):
